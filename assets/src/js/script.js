  ...
  var wb = {
    sampleFunction: {
      ...
      run: function run() {
        ...
        var originalPositions = divPosition;
        ...
        $('a[href^="#"]').on('click', function (e) {
          ...
          for(var i = 0; i < targetIndex; i++) {
            $('.scroll-item').eq(i).css('transform', 'translateY(' + -(originalPositions[i]['height']) + 'px)');
            $('.scroll-item').eq(i+1).css('transform', 'translateY(' + -(originalPositions[i+1]['height']) + 'px)');
          }
          $('.scroll-item:gt('+targetIndex+')').css('transform', 'translateY(0)')
          $('.scroll-item:lt('+targetIndex+')').css('transform', 'translateY(-1000)')
          $('.scroll-item').removeClass('active')
          $('.scroll-item').eq(i+1).addClass('active')
          $('.scroll-item').eq(i+1).next().addClass('active') 
          $('.scroll-overlay').css({
            zIndex: (divPosition.length - 1) - targetIndex
          });
          console.log((divPosition.length - 2) - targetIndex)
        });

        function runAnimation(targetIndex) {
          $('body').stop().animate({scrollTop:originalPositions[targetIndex+1]['top']}, 'slow');
